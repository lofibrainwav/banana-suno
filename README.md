...
## 프로젝트 소개
Jadaking Actor는 Apify 플랫폼에서 동작하는 Node.js 기반 스크래핑 및 자동화 Actor입니다. 모듈화된 구조와 미들웨어 파이프라인으로 확장성을 고려하여 설계되었습니다.
...
## 로컬 실행 방법
1. `npm install`으로 의존성을 설치합니다.
2. `.env.example` 파일을 복사하여 `.env`를 생성합니다.
3. 아래 스크립트로 서버를 실행합니다.
...
## .env 설정
`.env.example` 파일을 `.env`로 복사한 후 필요에 따라 값을 수정합니다.
